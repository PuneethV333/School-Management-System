...
import { getVal, scanKeys, setValKey } from "../utils/redis.utils";
import {
    addExamServices,
    addUtServices,
    validateData,
...
export const addUTMarks = async (req: Request, res: Response) => {
    try {
        ...
        const studentKeys = await scanKeys(`UT:${classNo}:${academicYear}:*`);
        await Promise.all([
            ...studentKeys.map((key) => redisClient.del(key)),
            redisClient.del(`UT:${classNo}:${academicYear}`),
        ]);
...
export const addExamMark = async (req: Request, res: Response) => {
    try {
        ...
        const studentKeys = await scanKeys(`Exam:${classNo}:${academicYear}:*`);
        await Promise.all([
            ...studentKeys.map((key) => redisClient.del(key)),
            redisClient.del(`Exam:${classNo}:${academicYear}:${payload.type}`),
        ]);
