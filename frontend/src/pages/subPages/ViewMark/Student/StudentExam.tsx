...
const StudentExam = () => {
  ...
  const { data: examData, isPending: loadingExamData } = useFetchExamMarks(
    classNo,
    userData,
  );
  const studentData = useMemo(() => {
    return Array.isArray(examData) ? examData[0] : examData;
  }, [examData]);

  if (loading || loadingExamData) {
    return <Spinner />;
  }
