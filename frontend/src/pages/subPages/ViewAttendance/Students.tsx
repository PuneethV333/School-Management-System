...
const Students = () => {
  ...
  const presentCount =
    filterStudents?.filter((s) => s.isPresentToday).length ?? 0;
  const totalCount = filterStudents?.length ?? 0;
        ...
        <div className="mb-10 flex flex-col sm:flex-row sm:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-2">
              Class {classNo} · {totalCount} enrolled
            </p>
          </div>
        ...
        <div className="bg-slate-900/80 border border-slate-700/40 rounded-2xl overflow-hidden shadow-2xl shadow-black/40 backdrop-blur-sm">
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800">
            <span className="text-slate-400 text-xs font-medium">
              ...
              {searchStudentName ? "results" : "students"}
            </span>
          ...
          {(filterStudents?.length ?? 0) > 0 && (
            <div className="px-5 py-3 border-t border-slate-800 flex justify-end">
              <span className="text-xs text-slate-600">
                {filterStudents?.length} of {res?.length} students
              </span>
            </div>
          )}
        </div>
